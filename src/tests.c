...
void test_basic_expressions() {
    ...
    // test_eval("((\\x.\\y.x) 42) 24", expected_int); INFINITE RECURSION.
    // PARSING ERR.
}
...
void test_higher_order() {
    ...
    Value expected_int = {.type = VAL_INT, .data = {.int_val = 42}};
    test_eval(
        "let compose = \\f.\\g.\\x.f (g x) in let double = \\x.add x x in "
        "let inc = \\x.add x 1 in ((compose double inc) 20)",
        expected_int);

    // Function passed as argument
    test_eval(
        "let apply = \\f.\\x.f x in let inc = \\x.add x 1 in apply inc 41",
        expected_int);
...
void test_multi_arg_application() {
    ...
    // Nested applications
    test_eval("(\\f.\\g.\\x.f (g x)) (\\y.add y 1) (\\z.multiply z 2) 2",
              expected_complex);
